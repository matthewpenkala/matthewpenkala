(function() {
    ...
    // This function handles the event and checks the device type
    function handleEventOnce() {
        // Remove all listeners to ensure the function runs only once
        window.removeEventListener('scroll', handleEventOnce);
        window.removeEventListener('click', handleEventOnce);
        window.removeEventListener('touchstart', handleEventOnce);
        // Continue JS execution only if the device type isn't a mobile
        if (!isMobileDevice()) {
            // Define a throttle function to limit the rate at which a function can fire.
            function throttle(func, delay) {
                let timeoutId;
                // Declare a variable for storing the timeout.
                let lastExecTime = 0;
                // Store the last execution time of the function.
                // Return a throttled version of the passed function.
                return function() {
                    const context = this;
                    // Preserve 'this' context from the original call.
                    const args = arguments;
                    // Preserve arguments passed to the function.
                    if (timeoutId) {
            ...
            $(document).ready(function() {
                $('*').scrollLeft(0);
                // Reset scroll position for all elements.
                // Apply a throttled scroll event listener to the document.
                $(document).on('scroll', throttle(function() {
                    $(this).scrollLeft(0);
                    // Reset scroll position on scroll event.
    ...
    // Add event listeners for scroll, click, and touchstart events
    window.addEventListener('scroll', handleEventOnce, {
        once: true
    });
    window.addEventListener('click', handleEventOnce, {
        once: true
    });
    window.addEventListener('touchstart', handleEventOnce, {
        once: true
    });
}());
