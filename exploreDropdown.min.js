$(function() {
    ...
    // Elements
    const $hoverable       = $("[class*='navbar-menu-item']").filter(":contains('Explore')")
                                .closest("[data-hover]")
                                .find("*:not(path)");
    ...
    const rotateArrow = (el, duration, open) => new Promise(resolve => {
        const [startAngle, endAngle] = open ? [0, 180] : [180, 0];
        const startTime = performance.now();

        const animate = now => {
            const progress     = Math.min((now - startTime) / duration, 1);
            const currentAngle = startAngle + (endAngle - startAngle) * ease(progress);
        ...
        mutations.forEach(mutation => {
            if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                ...
                if (!currentlyOpen && isArrowOpen) {
                    ...
                    setTimeout(() => {
                        if (!$dropdownList.hasClass(OPEN_CLASS) && isArrowOpen) setArrowState(false);
                    }, RECHECK_DELAY);
                }
            }
        });
