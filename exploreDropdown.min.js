$(document).ready(function() {
    ...
    const RECHECK_DELAY = 500; // 0.5 seconds
    ...
    // When 'w--open' is removed:
    //   1) Rotate arrow back immediately (original behavior)
    //   2) After 0.5 seconds, check again; if still not open, ensure arrow is back
    // -------------------------------------
    const observer = new MutationObserver(mutations => {
        for (const mutation of mutations) {
            if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                ...
                // If the dropdown just lost w--open and arrow is currently open
                if (!currentlyOpen && isArrowOpen) {
                    // Immediately rotate arrow back (original logic)
                    $toggleElement.removeClass("w-hover");
                    rotateArrow($arrowElement[0], ROTATION_DURATION, false).then(() => {
                        isArrowOpen = false;
                    });

                    // After 0.5s, re-check if still not open
                    setTimeout(() => {
                        const stillNotOpen = !$dropdownList.hasClass(OPEN_CLASS);
                        if (stillNotOpen && !isArrowOpen) {
                            // If by some chance it's stuck, force arrow back again
                            $toggleElement.removeClass("w-hover");
                            rotateArrow($arrowElement[0], ROTATION_DURATION, false).then(() => {
                                isArrowOpen = false;
                            });
